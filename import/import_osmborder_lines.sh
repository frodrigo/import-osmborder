...
function import_csv() {
    ...
    pgfutter \
        ...
        --table "$table_name" \
    csv \
        --fields "osm_id,admin_level,dividing_line,neutral,disputed,disputed_by,claimed_by,maritime,geometry" \
        --delimiter $'\t' \
    "$csv_file"
}
...
function generalize_border() {
    ...
    echo "CREATE TABLE $target_table_name AS SELECT ST_Simplify(geometry, $tolerance) AS geometry, osm_id, admin_level, dividing_line, neutral, disputed, disputed_by, claimed_by, maritime FROM $source_table_name WHERE admin_level <= $max_admin_level;" | exec_psql
...
function create_import_table() {
    local target_table_name="$1"
    echo "CREATE TABLE $target_table_name (osm_id bigint, admin_level int, dividing_line bool, neutral bool, disputed bool, disputed_by varchar[], claimed_by varchar[], maritime bool, geometry Geometry(LineString, 3857));" | exec_psql
    echo "CREATE INDEX ON $target_table_name USING gist (geometry);" | exec_psql
}
